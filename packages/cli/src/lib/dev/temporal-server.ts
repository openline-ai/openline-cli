...
function waitForTemporal(verbose: boolean) {
  ...
  const spinnerChars = ['|', '/', '-', '\\'];
  let spinnerIndex = 0;
  const spinnerColor = '\x1b[31m\x1b[1m'; // Yellow color (you can change the color code)
  ...
  do {
    ...
    if (verbose) {
      process.stdout.write(`\r${spinnerColor}${spinnerChars[spinnerIndex]}${resetColor} Waiting for temporal pod...`);
      spinnerIndex = (spinnerIndex + 1) % spinnerChars.length;
    }
  } while (temporalPodName.length < 1);

  if (verbose) {
    process.stdout.write('\r' + ' '.repeat(process.stdout.columns) + '\r'); // Clear the previous line
    console.log('✓ Temporal pod exists');
  }
